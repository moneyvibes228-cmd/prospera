...
import { ROLE_LABELS, ROLE_COLORS } from '@distributeur/lib/auth'
import { ROUTE_ACCESS } from '@distributeur/lib/route-access'
import { REGISTRE_RELANCES } from '@distributeur/lib/registries/relances-registry'
import { commandesEnAttenteValidation } from '@distributeur/lib/reappro-engine'
import { countProformasExpirantes } from '@distributeur/lib/registries/proformas-registry'
...
import { buildSyntheseAutomation } from '@distributeur/lib/automation/automation-types'
import { getPerimetreLogistique } from '@distributeur/lib/perimetre-logistique'
import { buildBonsPreparation } from '@distributeur/lib/picking-engine'
...
const NAV: NavGroup[] = [
  {
    group: 'Opérations',
    items: [
      { href: '/distributeur/pilotage-financier', label: 'Pilotage financier', icon: LineChart },
      { href: '/distributeur/dashboard', label: 'Tableau de bord', icon: LayoutDashboard,
        labelParRole: { COMMERCIAL: 'Ma journée', FREELANCE: 'Mon activité du jour' } },
      { href: '/distributeur/mon-activite', label: 'Mon agenda', icon: CalendarDays },
      { href: '/distributeur/tournees', label: 'Tournées & Cash', icon: Route },
      { href: '/distributeur/commandes', label: 'Commandes', icon: ShoppingCart,
        labelParRole: { COMMERCIAL: 'Mes commandes', FREELANCE: 'Mes commandes', PROSPECTION: 'Mes commandes' } },
      { href: '/distributeur/disponibilite', label: 'Disponibilité produits', icon: Package },
      { href: '/distributeur/entrepot', label: 'Opérations entrepôt', icon: Warehouse, badge: 'PREPARATION_DU_JOUR',
        labelParRole: { GEST_ENTREPOT: 'Mon entrepôt' } },
      { href: '/distributeur/stock', label: 'Stock & Logistique', icon: Package,
        labelParRole: { MARKETING: 'Produits à écouler', GEST_ENTREPOT: 'Stock de mon site' } },
      { href: '/distributeur/approvisionnement', label: 'Approvisionnement', icon: Truck, badge: 'APPRO_A_VALIDER' },
      { href: '/distributeur/facturation', label: 'Facturation & Proforma', icon: FileText, badge: 'PROFORMAS_EXPIRANTES',
        labelParRole: { COMMERCIAL: 'Mes proformas', FREELANCE: 'Mes proformas', PROSPECTION: 'Mes proformas' } },
      { href: '/distributeur/relances', label: 'Relances & Impayés', icon: Bell, badge: 'RELANCES_CRITIQUES',
  ...
  },
  {
    group: 'Commercial',
    items: [
      { href: '/distributeur/points-de-vente', label: 'Points de vente', icon: Store,
        labelParRole: {
      ...
      { href: '/distributeur/prospection', label: 'Conquête & Territoires', icon: Compass,
        labelParRole: { PROSPECTION: 'Ma conquête' } },
      { href: '/distributeur/commercial', label: 'Commercial terrain', icon: MapPin },
      { href: '/distributeur/marketing', label: 'Marketing & Campagnes', icon: Megaphone },
      { href: '/distributeur/marketing/social', label: 'Studio réseaux sociaux', icon: Share2 },
    ],
  },
  {
    group: 'Automatisation',
    items: [
      { href: '/distributeur/automatisations', label: 'Automatisations', icon: Bot, badge: 'AUTO_A_VALIDER' },
    ],
  },
  {
    group: 'Pilotage',
    items: [
      { href: '/distributeur/objectifs', label: 'Objectifs & Quotas', icon: Target },
      { href: '/distributeur/equipe', label: 'Équipe & Performance', icon: Trophy },
      { href: '/distributeur/comptabilite', label: 'Comptabilité', icon: Receipt },
    ],
  },
]
...
function filterNav(role: UserRole): NavGroup[] {
  return NAV.map(g => ({
    ...
    items: g.items
      .filter(item => {
        const routeKey = item.href.replace(/^\/distributeur/, '')
        const roles = ROUTE_ACCESS[routeKey]
          ...
          <div key={group.group} className="mb-4">
            ...
            {group.items.map(item => {
              const active = pathname === item.href
                ...
                || (item.href === '/distributeur/stock' && pathname === '/distributeur/catalogue')
