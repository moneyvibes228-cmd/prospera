...
export function canAccess(role: UserRole, pathname: string): boolean {
  // Le pathname réel est préfixé par /distributeur (montage dans le monorepo prospera-web).
  const path = pathname.startsWith('/distributeur') ? pathname.slice('/distributeur'.length) || '/' : pathname
