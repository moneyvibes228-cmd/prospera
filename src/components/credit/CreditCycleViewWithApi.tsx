...
import { ETAPE_CYCLE_ORDER, type EtapeCyclePret } from '@/lib/credit-cycle-hub'
import { useCreditCycleHubStrict } from '@/hooks/usePhasesAdStrict'
import {
  ApiErrorState,
  ApiLoadingState,
  ApiPageShell,
...
export function CreditCycleViewWithApi() {
  const { hub, state, error, reload } = useCreditCycleHubStrict()
  const [selected, setSelected] = useState('')

  if (state === 'loading') {
    return (
      <ApiPageShell title="Cycle de vie prêt" endpoint="GET /operations/credit-cycle">
        <ApiLoadingState label="Chargement cycle crédit…" />
      </ApiPageShell>
    )
  }
  ...
  if (state === 'error' || !hub) {
    return (
      <ApiPageShell title="Cycle de vie prêt" endpoint="GET /operations/credit-cycle" onRefresh={() => void reload()}>
        <ApiErrorState message={error ?? 'Erreur cycle crédit'} onRetry={() => void reload()} />
      </ApiPageShell>
    )
  }
  ...
  const dossierId = selected || hub.dossiers[0]?.id || ''
  const dossier = hub.dossiers.find((d) => d.id === dossierId) ?? hub.dossiers[0]

  return (
    <ApiPageShell
      title="Cycle de vie prêt"
      subtitle="Demandes, comité, décaissement et clôture — données backend."
      endpoint="GET /operations/credit-cycle"
    ...
    >
      ...
      <ModuleKpiGrid
        cols={6}
        items={[
          { label: 'Dossiers actifs', value: String(kpis.en_cours) },
          { label: 'Demandes/jour', value: String(kpis.demandes_jour), highlight: 'teal' },
          { label: 'Comité semaine', value: String(kpis.comite_semaine), highlight: 'blue' },
          { label: 'Décaiss. attente', value: String(kpis.decaissements_attente), highlight: 'orange' },
          { label: 'Clôtures/mois', value: String(kpis.clotures_mois) },
          { label: 'Délai moyen', value: `${kpis.delai_moyen_jours} j`, sub: 'obj. 9 j' },
        ]}
      />
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="space-y-2">
          ...
          <Link href="/credit/pipeline-with-api" className="block text-center text-sm text-teal-700 hover:underline py-2 cursor-pointer">
