...
import {
  ...
  SlidersHorizontal, ChevronDown, LayoutGrid, Clock,
} from 'lucide-react'
import {
  ...
  isPreCcAnalyseStage,
  isCcAnalyseStage,
  type RocPipelineCard, type RocPipelineStage, type RocPipelineStageId,
  ROC_STAGE_LABELS,
} from '@/lib/credit-pipeline-roc'
import { useCreditPipelineStrict } from '@/hooks/usePhasesAdStrict'
import {
  ApiErrorState,
  ApiLoadingState,
...
function SortableCard({ card, stageId, onOpen, onDecision, showActions }: SortableCardProps) {
...
export function PipelineCreditKanbanWithApi() {
  ...
  } = useCreditPipelineStrict(isRa ? AGENCE_RA.id : undefined)
  const [stages, setStages] = useState<RocPipelineStage[]>(apiStages)
  ...
  useEffect(() => {
    if (isRa || isCc) {
      setStages(
        AGENCE_PIPELINE_STAGE_IDS.map(id => {
          const fromApi = apiStages.find(st => st.id === id)
          return {
            id,
            label: AGENCE_PIPELINE_LABELS[id],
            accent: fromApi?.accent ?? 'bg-slate-500',
            cards: (fromApi?.cards ?? []).filter(
              c => !isRa || c.agence === raAgence || c.agence === '—',
            ),
          }
        }),
      )
    ...
    const rocIds = ['EN_ANALYSE', 'VALIDE_CHARGE', 'EN_ANALYSE_ROC', 'EN_COMITE_CREDIT', 'APPROUVE', 'DECAISSEMENT', 'EN_GESTION'] as const
    setStages(apiStages.filter(st => (rocIds as readonly string[]).includes(st.id)))
  }, [apiStages, isCc, isRa, raAgence])
  ...
  const openDossier = (card: RocPipelineCard, stageId: RocPipelineStageId) => {
    ...
    if (isCc && isCcAnalyseStage(stageId)) {
      router.push(`/credit/analyse-with-api?ref=${encodeURIComponent(card.reference)}`)
      return
    }
    router.push(`/credit/dossiers-with-api/${encodeURIComponent(id)}`)
  }
  ...
  if (pipelineState === 'loading') {
    return (
      <div className="p-8">
        <ApiLoadingState label="Chargement pipeline crédit…" />
      </div>
    )
  }
  ...
  if (pipelineState === 'error') {
    return (
      <div className="p-8">
        <ApiErrorState message={pipelineError ?? 'Erreur pipeline'} onRetry={() => void reloadPipeline()} />
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 h-[calc(100vh-8rem)]">
      ...
      <div className="shrink-0 bg-white border-y border-slate-200 px-4 py-2.5 flex flex-wrap items-center gap-3">
        ...
        {!isCc && !isRa && (
          <button
            ...
            className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg border cursor-pointer transition-colors ${
      ...
      <p className="shrink-0 text-[10px] text-slate-400 px-4 py-2 bg-white border-t border-slate-200">
        {isRa || isCc
          ? `Pipeline agence · ${AGENCE_PIPELINE_LABELS.SOUMIS} → ${AGENCE_PIPELINE_LABELS.VALIDE_CHARGE} · ${isRa ? `Périmètre ${raAgence} · ` : ''}Suivi jusqu'à validation CC`
          : `Pipeline ROC · ${ROC_STAGE_LABELS.EN_ANALYSE} → ${ROC_STAGE_LABELS.EN_GESTION} · Glissez-déposez · Cliquez → fiche processus crédit`}
      </p>
    </div>
  )
}
