...
import { CC_CALENDRIER_REFERENCE } from '@/lib/cc-credit-hub'
import { useDashboardChargeCreditStrict } from '@/hooks/usePhasesAdStrict'
import {
  ApiErrorState,
  ApiLoadingState,
  ApiPageShell,
...
export function DashboardChargeCreditWithApi() {
  const router = useRouter()
  const { hub, state, error, reload } = useDashboardChargeCreditStrict()
  const d = hub
  ...
  if (state === 'loading') {
    return (
      <ApiPageShell title="Tableau de bord CC" endpoint="GET /dashboard/charge-credit">
        <ApiLoadingState label="Chargement du dashboard…" />
      </ApiPageShell>
    )
  }
  ...
  if (state === 'error') {
    return (
      <ApiPageShell title="Tableau de bord CC" endpoint="GET /dashboard/charge-credit" onRefresh={() => void reload()}>
        <ApiErrorState message={error ?? 'Erreur dashboard'} onRetry={() => void reload()} />
      </ApiPageShell>
    )
  }
  ...
  return (
    <ApiPageShell
      title={`Bonjour ${hub.agent.nom.split(' ')[0]} — Analyse crédit`}
      subtitle={`Chargé de Crédit · ${d.mes_kpis.en_attente_total} dossiers en attente · objectif ${obj.decisions_cible} décisions · délai cible ${obj.delai_objectif_h} h`}
      endpoint="GET /dashboard/charge-credit"
      onRefresh={() => void reload()}
      actions={
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => router.push('/credit/dossiers-with-api')}
            className="inline-flex items-center gap-1.5 px-4 py-2.5 text-sm font-semibold text-white bg-teal-600 hover:bg-teal-700 rounded-xl transition-colors duration-200 cursor-pointer"
          >
            <FileText size={15} />
            Dossiers API
            <ArrowRight size={15} />
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <QuickLink href="/credit/pipeline" icon={Inbox} label="Pipeline crédit" desc="Soumis → Docs OK → Analyse → Validé CC" badge="4 étapes agence" />
