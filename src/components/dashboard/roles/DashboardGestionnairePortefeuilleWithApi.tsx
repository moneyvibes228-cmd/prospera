...
import { useDashboardGpStrict } from '@/hooks/usePhasesAdStrict'
import {
  ApiErrorState,
  ApiLoadingState,
  ApiPageShell,
...
export function DashboardGestionnairePortefeuilleWithApi() {
  const router = useRouter()
  const { hub, state, error, reload } = useDashboardGpStrict()
  const d = hub
  ...
  if (state === 'loading') {
    return (
      <ApiPageShell title="Portefeuille GP" endpoint="GET /dashboard/gestionnaire-portefeuille">
        <ApiLoadingState label="Chargement dashboard GP…" />
      </ApiPageShell>
    )
  }
  ...
  if (state === 'error') {
    return (
      <ApiPageShell title="Portefeuille GP" endpoint="GET /dashboard/gestionnaire-portefeuille" onRefresh={() => void reload()}>
        <ApiErrorState message={error ?? 'Erreur dashboard GP'} onRetry={() => void reload()} />
      </ApiPageShell>
    )
  }
  ...
  return (
    <ApiPageShell
      title={`Bonjour ${hub.agent.nom.split(' ')[0]} — Mon portefeuille`}
      subtitle={`${d.kpis_portefeuille.clients_actifs} clients · ${formatFcfa(d.kpis_portefeuille.valeur_portefeuille)} · PAR 30 ${d.kpis_qualite.par_30_pct}% · ${d.kpis_qualite.clients_en_retard} en retard`}
      endpoint="GET /dashboard/gestionnaire-portefeuille"
      onRefresh={() => void reload()}
      actions={<ExportButton label="Exporter portefeuille" filename="dashboard_gp" />}
    >
