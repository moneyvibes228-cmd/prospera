...
import { useDashboardRccStrict } from '@/hooks/usePhasesAdStrict'
import {
  ApiErrorState,
  ApiLoadingState,
  ApiPageShell,
} from '@/components/api-ui'
import { CollecteAgregatsBlockWithApi } from '@/components/collecte/CollecteAgregatsBlockWithApi'
...
export default function DashboardResponsableCommercialWithApi() {
  const { hub, state, error, reload } = useDashboardRccStrict()

  if (state === 'loading') {
    return (
      <ApiPageShell title="Dashboard commercial" endpoint="GET /dashboard/responsable-commercial">
        <ApiLoadingState label="Chargement dashboard RCC…" />
      </ApiPageShell>
    )
  }
  ...
  if (state === 'error' || !hub) {
    return (
      <ApiPageShell title="Dashboard commercial" endpoint="GET /dashboard/responsable-commercial" onRefresh={() => void reload()}>
        <ApiErrorState message={error ?? 'Erreur dashboard RCC'} onRetry={() => void reload()} />
      </ApiPageShell>
    )
  }
  ...
  return (
    <ApiPageShell
      title={`Bonjour ${(hub.agent?.nom ?? 'Responsable').split(' ')[0]} — Commercial & Collecte`}
      subtitle={`${hub.agent?.perimetre ?? 'Réseau'} · collecte ${kco?.taux_collecte_pct ?? 0}% · ${ke?.agents_actifs ?? 0} agents actifs · ${kc.nouveaux_clients_mois} nouveaux clients ce mois`}
      endpoint="GET /dashboard/responsable-commercial"
      onRefresh={() => void reload()}
      actions={<ExportButton label="Exporter rapport RCC" filename="dashboard_rcc" />}
    >
    <div className="space-y-5">
      <CollecteAgregatsBlockWithApi />
