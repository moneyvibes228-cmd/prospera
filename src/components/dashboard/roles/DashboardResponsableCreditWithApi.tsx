...
import { useDashboardRocStrict } from '@/hooks/usePhasesAdStrict'
import {
  ApiErrorState,
  ApiLoadingState,
  ApiPageShell,
...
export function DashboardResponsableCreditWithApi() {
  const router = useRouter()
  const { hub: d, state, error, reload } = useDashboardRocStrict()

  if (state === 'loading') {
    return (
      <ApiPageShell title="Dashboard ROC" endpoint="GET /dashboard/roc">
        <ApiLoadingState label="Chargement dashboard ROC…" />
      </ApiPageShell>
    )
  }
  ...
  if (state === 'error') {
    return (
      <ApiPageShell title="Dashboard ROC" endpoint="GET /dashboard/roc" onRefresh={() => void reload()}>
        <ApiErrorState message={error ?? 'Erreur dashboard ROC'} onRetry={() => void reload()} />
      </ApiPageShell>
    )
  }
  ...
  return (
    <ApiPageShell
      title="Bonjour Kafui — Pilotage Crédit & Opérations"
      subtitle={`${dossiersA48h} dossiers > 48h · ${alertesCritiques} alertes critiques`}
      endpoint="GET /dashboard/roc"
      onRefresh={() => void reload()}
      actions={<ExportButton label="Exporter synthèse ROC" filename="dashboard_roc" />}
    >
    <div className="space-y-5">
      <div className="flex flex-wrap gap-2">
        <button
          type="button"
          onClick={() => router.push('/credit/pipeline-with-api')}
          className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-white bg-orange-600 hover:bg-orange-700 rounded-lg transition-colors duration-200 cursor-pointer"
        >
          <BarChart3 size={13} />
          Pipeline crédit
          <ArrowRight size={13} />
        ...
        <button
          type="button"
          onClick={() => router.push('/credit/recouvrement-with-api')}
          className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-white bg-teal-600 hover:bg-teal-700 rounded-lg transition-colors duration-200 cursor-pointer"
        >
          <FileText size={13} />
          Recouvrement réseau
          <ArrowRight size={13} />
