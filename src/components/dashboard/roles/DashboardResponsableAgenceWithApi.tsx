...
import { useDashboardRaStrict } from '@/hooks/usePhasesAdStrict'
import {
  ApiErrorState,
  ApiLoadingState,
  ApiPageShell,
...
export function DashboardResponsableAgenceWithApi() {
  ...
  if (state === 'loading') {
    return (
      <ApiPageShell title="Dashboard agence" endpoint="GET /dashboard/responsable-agence">
        <ApiLoadingState label="Chargement dashboard agence…" />
      </ApiPageShell>
    )
  }
  ...
  if (state === 'error') {
    return (
      <ApiPageShell title="Dashboard agence" endpoint="GET /dashboard/responsable-agence" onRefresh={() => void reload()}>
  ...
  return (
    <ApiPageShell
      title={`Bonjour ${hub.agence.responsable.split(' ')[0]} — ${hub.agence.nom}`}
      subtitle={`${hub.pilotage.emprunteurs_credit} emprunteurs actifs · ${formatFcfa(d.kpis_activite.encours_agence)} encours · PAR 30j ${d.kpis_credit.par_30_pct}% · ${hub.pilotage.agents_terrain_actifs}/${hub.pilotage.agents_terrain} commerciaux terrain`}
      endpoint="GET /dashboard/responsable-agence"
      onRefresh={() => void reload()}
      actions={<ExportButton label="Exporter rapport agence" filename="rapport_agence" />}
    >
