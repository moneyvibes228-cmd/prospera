...
import type { MergedTerrainHome } from '@/lib/phases-ad-terrain-merge'
import { useDashboardTerrainStrict } from '@/hooks/usePhasesAdStrict'
import {
  ApiErrorState,
  ApiLoadingState,
  ApiPageShell,
...
//   MAIN COMPONENT
// =============================================================================
export default function DashboardTerrainWithApi() {
  const { hub: d, state, error, reload } = useDashboardTerrainStrict()
  const [openSynthese, setOpenSynthese] = useState(true)
  const [activeTab, setActiveTab] = useState<'tontine' | 'recouv' | 'prospect'>('recouv')

  if (state === 'loading') {
    return (
      <ApiPageShell title="Terrain — Ma journée" endpoint="GET /dashboard/terrain">
        <ApiLoadingState label="Chargement dashboard terrain…" />
      </ApiPageShell>
    )
  }
  ...
  if (state === 'error' || !d) {
    return (
      <ApiPageShell title="Terrain — Ma journée" endpoint="GET /dashboard/terrain" onRefresh={() => void reload()}>
        <ApiErrorState message={error ?? 'Erreur dashboard terrain'} onRetry={() => void reload()} />
      </ApiPageShell>
    )
  }
  ...
  return (
    <ApiPageShell
      title="Synthèse IA — Ma journée"
      subtitle={`${d.resume_journee.clientes_a_visiter} clientes · ${formatFcfa(d.resume_journee.montant_a_collecter)} à collecter · ${d.resume_journee.taux_atteinte_pct}% objectif`}
      endpoint="GET /dashboard/terrain"
      onRefresh={() => void reload()}
      actions={<ExportButton label="Exporter ma journée" filename="terrain_jour" />}
    >
...
function SectionRecouvrement({ data }: { data: MergedTerrainHome['recouvrement'] }) {
  return (
    <div className="space-y-4">
      {/* KPIs */}
...
// ─── Section TONTINES ─────────────────────────────────────────────────────
function SectionTontines({ data }: { data: MergedTerrainHome['tontines'] }) {
  return (
    <div className="space-y-4">
      {/* KPIs */}
...
// ─── Section PROSPECTION ──────────────────────────────────────────────────
function SectionProspection({ data }: { data: MergedTerrainHome['prospection'] }) {
  return (
    <div className="space-y-4">
      {/* KPIs */}
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5">
